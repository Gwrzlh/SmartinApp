...
<div class="min-h-screen bg-gray-50/50 p-4 sm:p-8">
    <div class="max-w-3xl mx-auto">
        <a href="{{ route('admin.mentor.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-amber-600 mb-6 transition-colors group">
            ...
            <div class="bg-gradient-to-r from-amber-600 to-amber-500 px-8 py-10 text-white">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-amber-50/80 text-sm mt-1">Ubah informasi profil dan keahlian {{ $mentor->mentor_name }}.</p>
                    </div>
                </div>
            </div>

            <div class="px-8 py-10">
                <form action="{{ route('admin.mentor.update', $mentor->id) }}" method="POST" class="space-y-6" id="mentorForm" onsubmit="handleFormSubmit(event)">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label for="mentor_name" class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Nama Lengkap Mentor</label>
                    ...
                    <div class="flex flex-col md:flex-row md:items-end gap-6 pt-4">
                        ...
                        <div class="flex items-center pb-3 px-4 h-[52px] bg-gray-50/50 border-2 border-gray-100 rounded-xl group hover:bg-amber-50/50 hover:border-amber-100 transition-all transition-all duration-300">
                            <input type="checkbox" name="isActive" id="isActive" class="h-5 w-5 text-amber-600 border-gray-300 rounded focus:ring-amber-500 cursor-pointer" {{ $mentor->is_active ? 'checked' : '' }}>
                            <label for="isActive" class="ml-3 text-sm font-semibold text-gray-600 cursor-pointer group-hover:text-amber-700">Status Mentor Aktif</label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3 pt-6 border-t border-gray-50">
                        ...
                        <div class="relative">
                            <select id="subjectSelector" class="block w-full px-4 py-3 rounded-xl border-2 border-gray-100 bg-gray-50 text-gray-700 transition-all focus:bg-white focus:border-amber-500 outline-none appearance-none cursor-pointer">
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-10 mt-6 border-t border-gray-50">
                        <a href="{{ route('admin.mentor.index') }}" class="px-6 py-3 text-sm font-bold text-gray-400 hover:text-red-500 transition-colors">Batal</a>
...
<script>
    ...
    function addTag(id, name) {
        ...
        tag.className = "group flex items-center bg-white border-2 border-amber-100 text-amber-700 pl-4 pr-2 py-2 rounded-xl shadow-sm hover:border-amber-300 transition-all animate-in";
    ...
    selector.addEventListener('change', function() {
        ...
        if (selectedSubjects.includes(id)) {
            Swal.fire({ icon: 'info', title: 'Sudah Ada', text: 'Mapel sudah masuk list.', confirmButtonColor: '#f59e0b' });
            this.value = "";
            return;
        }
    ...
    function handleFormSubmit(event) {
        ...
        if (selectedSubjects.length === 0) {
            Swal.fire({ icon: 'warning', title: 'Data Kurang', text: 'Pilih minimal satu keahlian.', confirmButtonColor: '#f59e0b' });
            return;
        }
