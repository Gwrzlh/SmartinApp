...
@section('content')
    ...
    <div class="min-h-screen bg-gray-50/50 p-4 sm:p-8">
        <div class="max-w-3xl mx-auto">
            <a href="{{ route('admin.bundling.index') }}"
                class="inline-flex items-center text-sm text-gray-500 hover:text-cyan-600 mb-6 transition-colors group">
            ...
            <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 overflow-hidden border border-gray-100">
                <div class="bg-gradient-to-r from-cyan-600 to-cyan-500 px-8 py-10 text-white">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h3 class="text-2xl font-bold tracking-tight">Edit Paket Bundling</h3>
                            <p class="text-cyan-50/80 text-sm mt-1">Perbarui informasi paket bundling yang sudah ada.</p>
                        </div>
                    </div>
                </div>

                <div class="px-8 py-10">
                    <form action="{{ route('admin.bundling.update', $bundling->id) }}" method="POST" id="bundlingForm"
                        ...
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            <div class="p-8 space-y-6">
                                ...
                                <div class="space-y-2">
                                    <label for="bundling_name"
                                        class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Nama Paket
                                        Bundling</label>
                                    <input type="text" name="bundling_name" id="bundling_name"
                                        ...
                                        placeholder="Contoh: Paket Intensif UN" required
                                        value="{{ old('bundling_name', $bundling->bundling_name) }}">
                                </div>

                                <div class="space-y-2">
                                    <label for="description"
                                        class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Deskripsi
                                        Paket</label>
                                    <textarea name="description" id="description" rows="3"
                                        class="block w-full px-4 py-3 rounded-xl border-2 border-gray-100 bg-gray-50/50 text-gray-700 transition-all focus:bg-white focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 outline-none placeholder:text-gray-300"
                                        placeholder="Jelaskan keuntungan paket ini..." required>{{ old('description', $bundling->description) }}</textarea>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        <label for="bundling_price"
                                            class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Harga
                                            Total Paket (Rp)</label>
                                        <div class="relative">
                                            <span
                                                class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400 font-bold">Rp</span>
                                            <input type="text" name="bundling_price" id="bundling_price"
                                                ...
                                                value="{{ old('bundling_price', number_format($bundling->bundling_price, 0, ',', '.')) }}">
                                        </div>
                                    </div>

                                    <div class="space-y-2">
                                        <label class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Status
                                            Keaktifan</label>
                                        <div class="flex items-center h-[52px]">
                                            <label class="relative inline-flex items-center cursor-pointer">
                                                <input type="checkbox" name="isActive" id="isActive" class="sr-only peer"
                                                    {{ old('isActive', $bundling->is_active) ? 'checked' : '' }}>
                                                <div
                                ...
                                <div class="space-y-3 pt-4 border-t border-gray-50">
                                    <label
                                        class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1 text-cyan-600">Pilih
                                        Anggota Mata Pelajaran</label>

                                    <div class="relative">
                                        <select id="subjectSelector"
                                            class="block w-full px-4 py-3 rounded-xl border-2 border-gray-100 bg-gray-50 text-gray-700 transition-all focus:bg-white focus:border-cyan-500 outline-none appearance-none cursor-pointer">
                                            <option value="" disabled selected>-- Klik untuk mencari/memilih mapel --
                                            </option>
                                            @foreach ($subjects as $subject)
                                                <option value="{{ $subject->id }}" data-name="{{ $subject->mapel_name }}">
                                                    {{ $subject->mapel_name }} (Rp
                                                    {{ number_format($subject->monthly_price, 0, ',', '.') }})
                                                </option>
                                            @endforeach
                                        </select>
                                        <div
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-gray-50">
                                        <div class="space-y-2">
                                            <label for="duration_months"
                                                class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Durasi
                                                (Bulan)</label>
                                            <div class="relative">
                                                <input type="number" name="duration_months" id="duration_months"
                                                    ...
                                                    required
                                                    value="{{ old('duration_months', $bundling->duration_mounths) }}">
                                                <span
                                                    class="absolute inset-y-0 right-4 flex items-center text-gray-400 text-sm italic">Bulan</span>
                                            </div>
                                        </div>

                                        <div class="space-y-2">
                                            <label for="start_date"
                                                class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Tanggal
                                                Mulai</label>
                                        ...
                                        <div class="space-y-2">
                                            <label for="capacity"
                                                class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Kapasitas
                                                (Slot)</label>
                                            <div class="relative">
                                                ...
                                                <span
                                                    class="absolute inset-y-0 right-4 flex items-center text-gray-400 text-sm italic">Siswa</span>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div
                                class="p-8 bg-gray-50/50 border-t border-gray-100 flex items-center justify-end space-x-4">
                                <a href="{{ route('admin.bundling.index') }}"
    ...
    <script>
        ...
        function handleFormSubmit(event) {
            ...
            // Validasi minimal 1 mapel untuk bundling
            if (selectedSubjects.length < 1) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Data Kurang',
                    text: 'Pilih minimal 1 mata pelajaran untuk membuat paket.',
        ...
        @if (session('success'))
            Swal.fire({
                ...
                window.location.href = "{{ route('admin.bundling.index') }}";
            });
        @endif
        ...
        // pre-populate selected subjects from backend or old input
        let selectedSubjects = @json(old('subjects_id', $selectedSubjects ?? []));

        // helper to add a tag and hidden input
        function addSubjectTag(id, name) {
            // avoid duplicates
            ...
            tag.className =
                "group flex items-center bg-white border-2 border-cyan-100 text-cyan-700 pl-4 pr-2 py-2 rounded-xl shadow-sm hover:border-cyan-300 transition-all animate-in fade-in zoom-in duration-300";
        ...
        // initialize tags from existing selection
        document.addEventListener('DOMContentLoaded', function() {
            if (selectedSubjects.length > 0) {
                placeholder.style.display = 'none';
                // copy current array and reset so addSubjectTag can push correctly
                const initial = selectedSubjects.slice();
                selectedSubjects = [];
                initial.forEach(function(id) {
                    const opt = selector.querySelector(`option[value="${id}"]`);
                    if (opt) {
                        addSubjectTag(id, opt.getAttribute('data-name'));
                    }
                });
            }
        });
        ...
        selector.addEventListener('change', function() {
            ...
            if (selectedSubjects.includes(id)) {
                Swal.fire({
                    icon: 'info',
                    title: 'Sudah Ada',
                    text: 'Mata pelajaran ini sudah masuk dalam list.',
                    confirmButtonColor: '#06b6d4'
                });
                this.value = "";
                return;
