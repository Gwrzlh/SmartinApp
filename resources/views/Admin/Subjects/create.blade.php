            ...
            <div class="bg-gradient-to-r from-cyan-600 to-cyan-500 px-8 py-10 text-white">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-cyan-50/80 text-sm mt-1">Daftarkan Mapel baru ke sistem SmartIn.</p>
                    </div>
                </div>
            </div>

            <div class="px-8 py-10">
                <form action="{{ route('admin.subjects.store') }}" method="POST" class="space-y-6" id="subjectsForm" onsubmit="handleFormSubmit(event)">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="mapel_name" id="mapel_name" 
                                ...
                                placeholder="ex: Matematika,Python,Gitar" required>
                         </div>
                            <div class="space-y-2">
...
<script>
    ...
    function handleFormSubmit(event) {
        ...
        if (!form.checkValidity()) {
            form.reportValidity();
            priceInput.value = new Intl.NumberFormat('id-ID').format(cleanValue);
            return;
        }
